...
## OWNER REVIEW FIX (erratum — OWNER-REVIEW-FIX-01)

Owner review before merge-to-main found two defects in the initial integration;
both were corrected as an erratum (history above is retained, not rewritten).

- **A. Service Agent server fail-closed adapter had a residual unsafe double
  cast.** `server/server.ts` originally constructed the fail-closed port inline
  with `} as unknown as ServiceAgentPort`. Corrected to an explicitly typed
  adapter (`const failClosedServiceAgent: ServiceAgentPort = { async run(_input:
  ServiceAgentRunInput): Promise<never> { ... } }`) — no cast, contract
  unchanged, fail-closed behavior unchanged (built-server consultation returns
  `SERVICE_AGENT_NOT_CONFIGURED` → FAILED, verified over HTTP).
- **B. Business Data seeded DEMO data was mislabelled `CONNECTED / READY /
  connected: true`.** `business-data-demo.ts` now honestly reports
  `DEMO / READY` with `health.connected: false`,
  `configuredResourceCount: 0`, `lastSuccessfulReadAt: null`,
  `lastReadbackStatus: NOT_RUN` (no fabricated provider resources, reads or
  connectivity). The browser presentation contract
  (`BusinessDataHealthView.mode`) was minimally widened to `'DEMO' | 'CONNECTED'`
  while the real Connected transport (`isBusinessDataEnvelope`) still REQUIRES
  `CONNECTED` and fails closed on any `DEMO` envelope (no Connected→DEMO
  fallback). The server seam stays `CONNECTED / BLOCKED` without a connected
  Feishu configuration.
- New dedicated identity suite
  `apps/operator-workspace/tests-workspace-ui/business-data-demo-identity.test.ts`
  (DEMO/READY/connected=false + `DEMO · READY` view label + no `CONNECTED · READY`)
  and a Connected-transport fail-closed test added to
  `src/features/business-data/business-data-client.test.ts`.
- `smoke-product-integration.mjs` Journey C now asserts the identity closure:
  browser surface `DEMO · READY` (+ `CONNECTED · READY` ABSENT) AND server seam
  `CONNECTED / BLOCKED` (`BUSINESS_DATA_NOT_CONFIGURED`).
- Tooling note: vitest 2.1.8 on this Windows host crashes (silent exit 1, no
  output) when the workspace UI + API suites share the `@busos/*` symlinked
  graph through `src/api.ts` + `src/build-info.ts`. Workaround (no dependency
  change): the operator test script now runs the two directories serially
  (`vitest run tests-workspace-ui && vitest run tests-workspace-api`) with a
  single-fork `vitest.config.ts`; all tests still run, none skipped.

Follow-up implementation commit (OWNER-REVIEW-FIX-01):
`86bd4b5ecb6347669d41a41c022c86e1d90a5ce7` (`86bd4b5`).
